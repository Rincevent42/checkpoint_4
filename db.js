...
class Database {
  init () {
    let config = {
      ...
      database: process.env.DB_NAME || 'modelcars',
      connectionLimit: 10,
    ...
    if (process.env.NODE_ENV === 'test') {
      config = {
        ...
        database: process.env.DB_NAME_TEST || 'modelcars_test',
        connectionLimit: 10,
        multipleStatements: true
      };
    }
  ...
  async getTableNames () {
    if (!this._tableNames) {
      this._tableNames = await this.query(`
          SELECT table_name
          FROM INFORMATION_SCHEMA.TABLES where table_schema = '${process.env.DB_NAME_TEST || 'modelcars_test'}' and table_name != 'migrations'
      `);
    }
